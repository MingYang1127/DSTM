...
class CDRDataLoader:
    ...
    def read_n_map(self):
        ...
        lengths, sents, self.documents, self.entities, self.pairs = \
            reader.read(self.input, self.documents, self.entities, self.pairs)
    ...
    def statistics(self):
        ...
        print('  Max Entity Count: {}'.format(max([len(e) for e in self.entities.values()])))

        doc_len = [sum([len(sent) for sent in doc]) for doc in self.documents.values()]
        print('  Documents exceeding 400 words Count: {}'.format(sum([1 for len_ in doc_len if len_ >= 400])))
