...
class BertSelfAttention(nn.Module):
    def __init__(self, config, entity_structure):
        ...
        # =================================dependency syntax information==============================
        self.weight_dependency_bili = nn.Parameter(nn.init.xavier_uniform_(
            torch.empty(self.num_attention_heads, self.attention_head_size, self.attention_head_size)))
        self.bias_bili = nn.Parameter(torch.zeros(self.num_attention_heads))
        self.weight_dependency_q = nn.Parameter(
            nn.init.xavier_uniform_(torch.empty(self.num_attention_heads, self.attention_head_size)))
        self.weight_dependency_k = nn.Parameter(
            nn.init.xavier_uniform_(torch.empty(self.num_attention_heads, self.attention_head_size)))
    ...
    def forward(
        ...
        attention_biass = torch.einsum("bnip,npq,bnjq->bnij", query_layer, self.weight_dependency_bili, key_layer)
        attention_biass += self.bias_bili[None, :, None, None]
        attention_biass = attention_biass * adj
        attention_scores += attention_biass

        # # Decomp Transformation
        # attention_bias_q = torch.einsum("bnid,nd->bni", query_layer, self.weight_dependency_k).unsqueeze(-1).repeat(1, 1, 1,
        #                                                                                                         query_layer.size(
        #                                                                                                             2))
        # attention_bias_k = torch.einsum("nd,bnjd->bnj", self.weight_dependency_q, key_layer).unsqueeze(-2).repeat(1, 1,
        #                                                                                                       key_layer.size(
        #                                                                                                           2), 1)
        # attention_scores += (attention_bias_q + attention_bias_k) * adj

        # # Direct Transformation
        # attention_biass = torch.matmul(adj, attention_scores)
        # attention_scores += attention_biass
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = self.self(
            layer_idx, hidden_states, attention_mask, head_mask, structure_mask, encoder_hidden_states, encoder_attention_mask, adj
        )
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(layer_idx, hidden_states, attention_mask, head_mask, structure_mask, encoder_hidden_states, encoder_attention_mask, adj)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(
                i, hidden_states, attention_mask, head_mask[i], structure_mask, encoder_hidden_states, encoder_attention_mask, adj
            )
