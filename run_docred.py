...
def load_and_cache_examples(args, tokenizer, evaluate=False, predict=False):
    ...
    if evaluate:
        ...
        file_dev_graph = open('D:/DSPM/data/bert_dev.graph', 'rb')
        ex_index2graph = pickle.load(file_dev_graph)
        file_dev_graph.close()
    elif predict:
        examples = processor.get_test_examples(args.data_dir)

        file_test_graph = open('D:/DSPM/data/bert_test.graph', 'rb')
        ex_index2graph = pickle.load(file_test_graph)
        file_test_graph.close()
    else:
        examples = processor.get_train_examples(args.data_dir)

        file_train_graph = open('D:/DSPM/data/bert_train_annotated.graph', 'rb')
    ...
    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids,
                            ...
                            all_structure_mask, all_label, all_label_mask, all_adj)
